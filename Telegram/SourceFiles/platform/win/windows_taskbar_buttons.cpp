...
void TaskbarButtons::apply(State state, bool create) {
	...
	FillButton(
		...
		ButtonId::Previous,
		_previousIcon,
		tr::lng_media_player_button_previous(tr::now),
		state.active,
	...
	FillButton(
		...
		(state.playing
			? tr::lng_media_player_button_pause(tr::now)
			: tr::lng_media_player_button_play(tr::now)),
		state.active,
		true);
	FillButton(
		buttons[2],
		ButtonId::Next,
		_nextIcon,
		tr::lng_media_player_button_next(tr::now),
		state.active,
		state.nextAvailable);
